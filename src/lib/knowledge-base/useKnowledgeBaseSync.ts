// Sync hook: base de conhecimento da IA (ai_knowledge_entries).
import { useEffect, useRef } from "react";
import { useServerFn } from "@tanstack/react-start";
import { getActiveCompanyId } from "@/lib/company-scope";
import {
  hydrateKnowledgeBaseFromDb,
  markKnowledgeBaseSyncError,
} from "@/lib/knowledge-base";
import { listKbEntriesDb, type KbEntryDto } from "@/lib/knowledge-base/kb.functions";

const UUID_RE =
  /^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;

export function useKnowledgeBaseSync() {
  const listFn = useServerFn(listKbEntriesDb);
  ...
  useEffect(() => {
    ...
    async function sync() {
      ...
      try {
        ...
        if (cancelled) return;
        hydrateKnowledgeBaseFromDb(companyId, rows as KbEntryDto[]);
      } catch (err) {
        if (cancelled) return;
        markKnowledgeBaseSyncError(
          err instanceof Error ? err.message : "Falha ao sincronizar base de conhecimento",
        );
      }
    }
    void sync();
