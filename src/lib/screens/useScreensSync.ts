// Hook que sincroniza o cache local de telas/credenciais com o banco.
// Garante que MAC, KEY, usuário, senha, servidor vinculado e expiração
// apareçam iguais no desktop, celular e PWA.
import { useEffect, useRef } from "react";
import { useServerFn } from "@tanstack/react-start";
import { getActiveCompanyId } from "@/lib/company-scope";
import { hydrateScreensFromDb, markScreensSyncError } from "@/lib/app-screens";
import { listScreensDb, type ScreenDto } from "@/lib/screens/screens.functions";

const UUID_RE =
  /^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;

export function useScreensSync() {
  const listFn = useServerFn(listScreensDb);
  ...
  useEffect(() => {
    ...
    async function sync() {
      ...
      try {
        ...
        if (cancelled) return;
        markScreensSyncError(
          err instanceof Error ? err.message : "Falha ao sincronizar telas",
        );
      }
    }
