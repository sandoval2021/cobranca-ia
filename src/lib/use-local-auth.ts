...
export function useLocalAuth() {
  ...
  useEffect(() => {
    ...
    // Deps: APENAS o que afeta a identidade/role do usuário bridged.
    // nome/whatsapp NÃO entram aqui — eles vêm de user_metadata e podem
    // mudar de referência sem mudar valor, o que reabria o ciclo
    // setBridgedLocalUser → emitChange → refresh → setLocalUser → re-render
    // (Maximum update depth exceeded). setBridgedLocalUser já tem guard
    // interno que só emite quando id/role realmente mudam.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [supaUser?.email, supaUser?.id, user?.id, user?.role, localUser?.id]);
