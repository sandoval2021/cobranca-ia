-- H1 — Backend-first /empresas + entitlements + active_company (retry com cast)
...
CREATE POLICY "spe_select_authenticated" ON public.saas_plan_entitlements
  FOR SELECT TO authenticated USING (true);
DROP TRIGGER IF EXISTS trg_spe_touch_updated_at ON public.saas_plan_entitlements;
CREATE TRIGGER trg_spe_touch_updated_at BEFORE UPDATE ON public.saas_plan_entitlements
...
CREATE TRIGGER trg_ucp_touch_updated_at BEFORE UPDATE ON public.user_company_preferences
  FOR EACH ROW EXECUTE FUNCTION public.touch_updated_at();

-- 4) Seeds (com cast explícito)
WITH plans AS (SELECT id, slug, ai_monthly_limit FROM public.saas_plans)
INSERT INTO public.saas_plan_entitlements (plan_id, feature_key, enabled, limit_value)
SELECT id, 'dns'::text, false, NULL::integer FROM plans
UNION ALL
SELECT id, 'ia'::text,  false, NULL::integer FROM plans WHERE slug IN ('essencial','profissional')
UNION ALL
SELECT id, 'ia'::text,  true,  ai_monthly_limit::integer FROM plans WHERE slug = 'escala'
ON CONFLICT (plan_id, feature_key) DO UPDATE
  SET enabled = EXCLUDED.enabled,
      limit_value = EXCLUDED.limit_value,
      updated_at = now();

-- 5) Backfill membership
INSERT INTO public.company_members (company_id, user_id, role)
SELECT c.id, c.owner_id, 'owner' FROM public.companies c WHERE c.owner_id IS NOT NULL
...
BEGIN
  ...
  SELECT active_company_id INTO v_id FROM public.user_company_preferences WHERE user_id=v_uid;
  IF v_id IS NOT NULL AND NOT public.has_company_access(v_id) THEN v_id := NULL; END IF;
  IF v_id IS NULL THEN
    SELECT cm.company_id INTO v_id FROM public.company_members cm
     WHERE cm.user_id=v_uid ORDER BY cm.created_at ASC NULLS LAST LIMIT 1;
  END IF;
  RETURN v_id;
END; $fn$;
...
BEGIN
  IF v_uid IS NULL THEN RAISE EXCEPTION 'not_authenticated' USING ERRCODE='28000'; END IF;
  IF p_company_id IS NULL THEN RAISE EXCEPTION 'company_required' USING ERRCODE='22023'; END IF;
  IF NOT public.has_company_access(p_company_id) THEN RAISE EXCEPTION 'forbidden_company' USING ERRCODE='42501'; END IF;
...
BEGIN
  ...
  IF public.is_super_admin() THEN
    SELECT COALESCE(jsonb_agg(jsonb_build_object(
      ...
      'archived_at',c.archived_at,'archived_by',c.archived_by,'created_at',c.created_at
...
BEGIN
  IF v_uid IS NULL THEN RAISE EXCEPTION 'not_authenticated' USING ERRCODE='28000'; END IF;
  IF NOT public.is_super_admin() THEN RAISE EXCEPTION 'forbidden_super_admin_only' USING ERRCODE='42501'; END IF;
  IF p_name IS NULL OR length(btrim(p_name))=0 THEN RAISE EXCEPTION 'name_required' USING ERRCODE='22023'; END IF;
  IF p_owner_id IS NULL THEN RAISE EXCEPTION 'owner_required' USING ERRCODE='22023'; END IF;
  INSERT INTO public.companies (name, owner_id) VALUES (btrim(p_name), p_owner_id) RETURNING id INTO v_id;
  INSERT INTO public.company_members (company_id, user_id, role) VALUES (v_id, p_owner_id, 'owner')
  ON CONFLICT (company_id, user_id) DO NOTHING;
  RETURN v_id;
END; $fn$;

CREATE OR REPLACE FUNCTION public.update_company_admin(p_company_id uuid, p_name text, p_owner_id uuid)
RETURNS void LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $fn$
DECLARE v_uid uuid := auth.uid();
BEGIN
  IF v_uid IS NULL THEN RAISE EXCEPTION 'not_authenticated' USING ERRCODE='28000'; END IF;
...
BEGIN
  ...
  IF NOT public.is_super_admin() THEN RAISE EXCEPTION 'forbidden_super_admin_only' USING ERRCODE='42501'; END IF;
  IF p_company_id IS NULL THEN RAISE EXCEPTION 'company_required' USING ERRCODE='22023'; END IF;
  UPDATE public.companies SET archived_at=now(), archived_by=v_uid, updated_at=now() WHERE id=p_company_id;
END; $fn$;

CREATE OR REPLACE FUNCTION public.get_company_entitlements(p_company_id uuid)
RETURNS jsonb LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path = public AS $fn$
DECLARE v_uid uuid := auth.uid(); v_plan_id uuid; v_rows jsonb;
BEGIN
  IF v_uid IS NULL THEN RAISE EXCEPTION 'not_authenticated' USING ERRCODE='28000'; END IF;
  IF p_company_id IS NULL THEN RAISE EXCEPTION 'company_required' USING ERRCODE='22023'; END IF;
  IF NOT public.has_company_access(p_company_id) THEN RAISE EXCEPTION 'forbidden_company' USING ERRCODE='42501'; END IF;
  SELECT plan_id INTO v_plan_id FROM public.company_subscriptions WHERE company_id=p_company_id;
  IF v_plan_id IS NULL THEN RETURN jsonb_build_object('plan_id', NULL, 'entitlements', '[]'::jsonb); END IF;
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    'feature_key',e.feature_key,'enabled',e.enabled,'limit_value',e.limit_value
  ) ORDER BY e.feature_key), '[]'::jsonb)
  INTO v_rows FROM public.saas_plan_entitlements e WHERE e.plan_id=v_plan_id;
